...
- **정의**: `수주 / 전체 리드 × 100` (기간 필터 필요).
- **주의**: 리드가 실제로 클로즈될 때까지 시간 걸리므로 최근 30일 리드로 CVR 계산하면 과소평가.
- **권장**: 리드 생성일 기준 90일 이상 지난 것만 CVR 계산에 포함.
...
- **정의**: 광고 캠페인으로 인한 매출 ÷ 광고비.
- **매출 귀속**: `pipeline_leads.first_touch.utm_campaign` → 그 리드가 수주된 프로젝트 매출.
- **주의**: first-touch 기준 (last-touch 아님). 정책 명확히 유지.
