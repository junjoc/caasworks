...
- **식별**: `projects.sheet_year = YEAR` AND `revenues.length = 0`.
- **UI 표시** (팀 결정 2026-07-03):
  - 해당 프로젝트에 **회색 "연간계약" 배지** 표시
  - 매출 페이지 상단에 **"매출 있는 것만" 필터 토글** 제공 (기본 OFF = 다 표시)
...
- **실제 코드 8단계** (`pipeline/list/page.tsx`, `pipeline/board/page.tsx`):
  ...
  7. `도입완료` — 수주 (계약 성사)
  8. `이탈` — 실패/거절
- **결정 (팀 2026-07-03, Q4)**: 국장 설계대로 현재 순서 `예정 → 제안 → 미팅 → 도입직전` **유지**.

### 2.4 CVR — 도입률 vs 승률 (팀 권고 2026-07-03)
전환율을 **두 지표로 분리**하여 병기, 화면별 기본값 다르게 표시.

**(a) 도입률 (Adoption Rate)**
- `도입률 = 도입완료 / 전체 리드 × 100`
- 분모: 전체 리드 (진행중/보류/이탈 포함)
- 의미: 리드 총량 대비 최종 수주율. 최근 코호트 반영 빠름.
- **기본 표시**: 대시보드, 마케팅 관련 페이지

**(b) 승률 (Win Rate)**
- `승률 = 도입완료 / (도입완료 + 이탈) × 100`
- 분모: 결과가 나온 리드 (결판)
- 의미: 클로즈된 리드의 성공률. 진행중 제외.
- **기본 표시**: 세일즈 파이프라인 관련 페이지 (기본 확정은 국장 승인 대기)

**국장 확정 대기**: 마케팅=도입률 / 세일즈=승률 배치 방향.

### 2.4b 사이트 전환율 (Site→Inquiry Rate)
- **정의** (`marketing/ads/page.tsx`):
  - `사이트 전환율 = 문의클릭수 / GA사이트유입수 × 100`
- **의미**: 사이트 방문 중 얼마나 문의로 이어졌는가 (랜딩페이지/CTA 성능).
- **주의**: 도입률(2.4)·승률과 이름 구분. 팀 결정 3분화: 사이트 전환율 / 도입률 / 승률.
...
- **팀 권고 (2026-07-03)**:
  - **선행 조건**: 리드↔프로젝트↔고객 자동연결 (P0-6과 한 세트)
  - **귀속 정책**: **first-touch 기준**
  - **시차 반영**: **코호트 기준** (광고비-매출 발생 시점 차이)
  - **정책**: 자동연결 완료 전까진 ROAS 미산출 유지
- **국장 확정 대기**: 착수 시점.
- **정의 (참고)**: `ROAS = 광고 기인 매출 ÷ 광고비`
  - 광고 기인 매출 = first-touch utm_campaign 리드가 수주 → 그 프로젝트 매출 합산
