...
export default function RevenuePage() {
  ...
  // 편집: 딱 1개 셀만
  const [ec, setEc] = useState<{ rid: string; f: Field } | null>(null)
  const [ev, setEv] = useState('')
  // 매출 편집: 딱 1개 셀만
  const [er, setEr] = useState<{ rid: string; m: number } | null>(null)
  ...
  const totals = useMemo(() => {
    ...
    filtered.forEach(r => (r.revenues||[]).forEach(v => { t[v.month] = (t[v.month]||0) + Number(v.amount); g += Number(v.amount) }))
    return { t, g }
  }, [filtered])

  /* ── Field save (optimistic) ── */
  const saveField = useCallback(async () => {
    if (!ec) return
    const { rid, f } = ec; const val = ev.trim()
  ...
  /* ── Revenue save (optimistic) ── */
  const saveRev = useCallback(async () => {
    if (!er) return
    const { rid, m } = er; const amt = Number(erv)
    const row = rows.find(r => r.id === rid)
    if (!row) { setEr(null); return }
    const existing = (row.revenues||[]).find(v => v.month === m)

    if (existing) {
      if (isNaN(amt) || amt <= 0) {
        setRows(prev => prev.map(r => r.id === rid ? { ...r, revenues: r.revenues.filter(v => v.id !== existing.id) } : r))
        setEr(null); setErv('')
        await sb.from('monthly_revenues').delete().eq('id', existing.id)
        toast.success('삭제'); return
      }
      setRows(prev => prev.map(r => r.id === rid ? { ...r, revenues: r.revenues.map(v => v.id === existing.id ? { ...v, amount: amt } : v) } : r))
      setEr(null); setErv('')
      const { error } = await sb.from('monthly_revenues').update({ amount: amt }).eq('id', existing.id)
      if (error) { toast.error('실패'); load() }
    } else {
      if (isNaN(amt) || amt <= 0) { setEr(null); return }
      const tid = `t${Date.now()}`
      setRows(prev => prev.map(r => r.id === rid ? { ...r, revenues: [...r.revenues, { id: tid, month: m, amount: amt, is_confirmed: false }] } : r))
      setEr(null); setErv('')
      const { data, error } = await sb.from('monthly_revenues').insert({ customer_id: row.customer_id, project_id: rid, year, month: m, amount: amt, is_confirmed: false }).select('id').single()
      ...
      toast.success(`${m}월 매출 등록`)
    }
  }, [er, erv, rows, sb, year, load])

  /* ── Row actions ── */
  const doCopy = useCallback((r: Row) => { setCopyFrom(r); setModal(true) }, [])
  ...
  /* ── Cell click handlers ── */
  const clickField = (rid: string, f: Field, val: string) => { setEc({ rid, f }); setEv(val); setEr(null) }
  const clickRev = (rid: string, m: number, rev?: Rev) => { setEr({ rid, m }); setErv(rev ? String(rev.amount) : ''); setEc(null) }
  const cancelField = () => { setEc(null); setEv('') }
  const cancelRev = () => { setEr(null); setErv('') }
      ...
      {loading ? <Loading /> : (
        <div className="card rt overflow-auto" style={{ maxHeight: 'calc(100vh - 280px)' }}>
          <table className="w-full text-xs border-collapse">
            ...
            <tbody>
              {filtered.map((r, i) => {
                const rm: Record<number, Rev> = {}
                let rt = 0
                ;(r.revenues||[]).forEach(v => { rm[v.month] = { ...v, amount: Number(v.amount) }; rt += Number(v.amount) })

                return (
                  <tr key={r.id} className="border-b border-gray-200 hover:bg-blue-50/30 group">
                    <td className={`px-1.5 py-1.5 ${B} text-center font-bold text-gray-700 sticky left-0 bg-white group-hover:bg-blue-50/30 z-10`}>{i+1}</td>
                    <td className={`px-0.5 py-1 ${B} text-center sticky left-[36px] bg-white group-hover:bg-blue-50/30 z-10`}>
                      <div className="flex items-center justify-center gap-0.5">
                        <button onClick={() => doCopy(r)} className="p-1 text-gray-400 hover:text-primary-600 hover:bg-primary-50 rounded" title="서비스 추가"><Copy className="w-3.5 h-3.5" /></button>
                        <button onClick={() => doDelete(r.id)} className="p-1 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded" title="삭제"><Trash2 className="w-3.5 h-3.5" /></button>
                      </div>
                    </td>

                    {/* 시작일 */}
                    <td className={`px-1.5 py-1.5 ${B} text-center`}>
                      {ec?.rid===r.id && ec.f==='project_start' ? (
                        ...
                        <div className="cursor-pointer min-h-[24px] flex items-center justify-center" onClick={()=>clickField(r.id,'project_start',r.project_start||'')}>
                    ...
                    {/* 종료일 */}
                    <td className={`px-1.5 py-1.5 ${B} text-center`}>
                      {ec?.rid===r.id && ec.f==='project_end' ? (
                        <input type="date" className="w-full text-xs border border-primary-400 rounded px-0.5 py-1 focus:outline-none focus:ring-1 focus:ring-primary-500" value={ev} onChange={e=>setEv(e.target.value)} onBlur={saveField} onKeyDown={e=>{if(e.key==='Enter')saveField();if(e.key==='Escape')cancelField()}} autoFocus />
                      ) : (
                        <div className="cursor-pointer min-h-[24px] flex items-center justify-center" onClick={()=>clickField(r.id,'project_end',r.project_end||'')}>
                    ...
                    <td className={`px-1.5 py-1.5 ${B} text-left max-w-[220px]`}>
                      {ec?.rid===r.id && ec.f==='project_name' ? (
                        ...
                        <div className="cursor-pointer min-h-[24px] flex items-center truncate" onClick={()=>clickField(r.id,'project_name',r.project_name||'')} title={r.project_name}>
                      ...
                      return (
                        <td key={f} className={`px-1.5 py-1.5 ${B} text-center ${f==='billing_method'?'text-[10px]':''}`}>
                          {isEdit ? (
                            ...
                            <div className="cursor-pointer min-h-[24px] flex items-center justify-center" onClick={()=>clickField(r.id,f,val||'')}>
                              {f==='service_type' && val ? (
                    ...
                    {/* 과금 시작일 */}
                    <td className={`px-1.5 py-1.5 ${B} text-center`}>
                      {ec?.rid===r.id && ec.f==='billing_start' ? (
                        <input type="date" className="w-full text-xs border border-primary-400 rounded px-0.5 py-1 focus:outline-none focus:ring-1 focus:ring-primary-500" value={ev} onChange={e=>setEv(e.target.value)} onBlur={saveField} onKeyDown={e=>{if(e.key==='Enter')saveField();if(e.key==='Escape')cancelField()}} autoFocus />
                      ) : (
                        <div className="cursor-pointer min-h-[24px] flex items-center justify-center" onClick={()=>clickField(r.id,'billing_start',r.billing_start||'')}>
                    ...
                    {/* 과금 종료일 */}
                    <td className={`px-1.5 py-1.5 ${B} text-center`}>
                      {ec?.rid===r.id && ec.f==='billing_end' ? (
                        <input type="date" className="w-full text-xs border border-primary-400 rounded px-0.5 py-1 focus:outline-none focus:ring-1 focus:ring-primary-500" value={ev} onChange={e=>setEv(e.target.value)} onBlur={saveField} onKeyDown={e=>{if(e.key==='Enter')saveField();if(e.key==='Escape')cancelField()}} autoFocus />
                      ) : (
                        <div className="cursor-pointer min-h-[24px] flex items-center justify-center" onClick={()=>clickField(r.id,'billing_end',r.billing_end||'')}>
                    ...
                    {MS.map(m => {
                      ...
                      const isEdit = er?.rid===r.id && er.m===m
                      return isEdit ? (
                        ...
                        <td key={m} className={`px-1 py-1.5 ${B} text-right cursor-pointer hover:bg-primary-50 ${cur?'bg-red-50/20':''}`} onClick={()=>clickRev(r.id,m,v)}>
