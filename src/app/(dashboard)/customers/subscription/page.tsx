...
import { Users, RefreshCw, AlertTriangle, TrendingUp, Search, Pencil, Plus, CreditCard, Calendar } from 'lucide-react'
          ...
          <table className="data-table" style={{ minWidth: '900px' }}>
            <thead>
              <tr>
                ...
                <th style={{ width: '9%' }}>과금유형</th>
                <th style={{ width: '14%' }}>서비스</th>
                <th style={{ width: '12%' }} className="text-right">월 과금액</th>
                <th style={{ width: '8%' }} className="text-center">프로젝트</th>
                <th style={{ width: '16%' }} className="text-center">과금기간</th>
                <th style={{ width: '8%' }} className="text-center">갱신까지</th>
                <th style={{ width: '7%' }} className="text-center">이탈위험</th>
                <th style={{ width: '5%' }} className="text-center">관리</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((cust) => (
                <tr key={cust.id}>
                  <td className="col-company">
                    <Link href={`/customers/${cust.id}`} className="font-medium text-primary-400 hover:text-primary-500 hover:underline truncate block">{cust.company_name}</Link>
                  ...
                  <td className="text-text-secondary col-truncate text-caption">{cust.service_type || '-'}</td>
                  ...
                  <td className="col-status">
                    {cust.days_until_renewal !== null ? (
                      <span className={`text-body-sm font-medium ${cust.days_until_renewal <= 30 ? 'text-status-red' : cust.days_until_renewal <= 90 ? 'text-status-yellow' : 'text-text-secondary'}`}>
                        {cust.days_until_renewal > 0 ? `${cust.days_until_renewal}일` : '만료됨'}
                      </span>
                    ) : '-'}
                  </td>
                  <td className="col-status">
